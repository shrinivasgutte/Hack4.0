...
public class MEDICAL_Image_Adapter extends ArrayAdapter<String> {
   ...
   public MEDICAL_Image_Adapter(Activity context, String[]pdfName){
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
       ...
       //         .execute(State_and_City.url+"Images/"+State_and_City.patientNameSend_server+"/Medical/"+HomeTab_3.Date_Temp.replace("/", "") + "/"+pdfName[position].replace(" ", "%20"));
        new DownloadImageTask((ImageView) rowView.findViewById(R.id.cp_img))
                .execute(State_and_City.url_patient_report+ State_and_City.patientNameSend_server+"/Medical/"+HomeTab_3.Date_Temp.replaceAll("\\W","") + "/"+pdfName[position].replace(" ", "%20"));
